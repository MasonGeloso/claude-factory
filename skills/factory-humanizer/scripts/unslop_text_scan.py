...
measure rhythm or emptiness, so those are reported in references/english.md for a human pass,
...
RULES = [
    ...
    # The bare form, with no "just"/"only" hedge to key on. Same negate-then-assert accent,
    # and the hedged patterns above miss it entirely. Two shapes, both scoped tightly to the
    # rhetorical-flourish version so the ordinary instructional "do X, not Y" is left alone:
    # a short noun-phrase pair CLOSING a sentence, and "is not a X. It's a Y." across a full stop.
    {"id": "bare-antithesis", "label": "Bare \"X, not Y\" / \"is not a X. It's a Y.\" antithesis flourish", "sev": "medium",
     "share": "structural sibling of not-just-x-y",
     "fix": "Assert the thing positively and drop the foil. If the contrast is load-bearing, give the rejected reading its own sentence and a reason.",
     "pats": [r",\s+not\s+(?:a|an|the)\s+[\w-]+(?:\s+(?:and|or)\s+(?:a|an|the)\s+[\w-]+)?\s*[.!?]",
              r"\b(?:is|was|are|were|it'?s|that'?s)\s+not\s+(?:a|an)\s+[\w-]+(?:\s+[\w-]+)?\s*[.!]\s+(?:It'?s|That'?s|They'?re|It is)\b"]},
...
def main():
    ...
    if not findings:
        print("  Nothing flagged. Either it is clean or the tells are structural ones a regex\n"
              ...
              "  and check those by eye against references/english.md.\n")
        return
    for rid in rule_ids:
        items = by_rule[rid]
        f0 = items[0]
    ...
    print("  Structural tells (rhythm, sycophancy, emptiness) need a human pass. See references/english.md.\n")
    sys.exit(by_sev.get("high", 0))

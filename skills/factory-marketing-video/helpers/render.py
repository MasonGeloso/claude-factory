...
def extract_segment(
    ...
    Quality ladder:
      ...
      - native:          source resolution and fps, untouched (see below)

    `native` exists for sources already below 1080p -- screen recordings especially.
    Upscaling a 720p capture to 1080p resamples every glyph on screen and makes UI
    text mushy, and forcing 60fps screen content to 24 makes cursor movement and
    scrolling judder. Neither is a quality gain, so for such sources pass native
    and let the source spec stand.
    """
    ...
    if vf:                      # native + no grade + no tonemap leaves nothing to filter
        cmd += ["-vf", vf]
    cmd += [
        "-af", af,
    ...
    if not native:              # native keeps the source frame rate
        cmd += ["-r", "24"]
    cmd += [
...
def extract_all_segments(
    ...
    for i, r in enumerate(ranges):
        ...
        extract_segment(src_path, start, duration, seg_filter, out_path,
                        preview=preview, draft=draft, native=native)
...
def main() -> None:
    ...
    ap.add_argument(
        "--native",
        action="store_true",
        help="Keep the source resolution and frame rate. Use for sources already "
             "below 1080p (screen recordings) where upscaling only softens detail.",
    )
    ap.add_argument(
        "--draft",
        action="store_true",
    ...
    segment_paths = extract_all_segments(
        edl, edit_dir, preview=args.preview, draft=args.draft, native=args.native
    )
